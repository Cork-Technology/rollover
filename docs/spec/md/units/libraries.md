...
| `MODULE_TYPE_PRE_ROLLOVER_HOOK` | `ModuleType` | `0xc0c0_0001` | `Typehashes.sol:46` |
| `MODULE_TYPE_MID_ROLLOVER_HOOK` | `ModuleType` | `0xc0c0_0002` | `Typehashes.sol:49` |
| `MODULE_TYPE_POST_ROLLOVER_HOOK` | `ModuleType` | `0xc0c0_0003` | `Typehashes.sol:52` |
| `MODULE_TYPE_EXECUTOR` | `ModuleType` | `0xc0c0_0004` | `Typehashes.sol:55` |
...
- **`MODULE_TYPE_*` discriminator reuse** — the four `ModuleType` constants `0xc0c0_0001..0xc0c0_0004` (`Typehashes.sol:45-55`) are opaque buckets layered atop ERC-7484. Reusing a numeric value for a different role would let a module legitimately attested for one phase be invoked as another (the precise vector per-phase attestation was designed to close). Keep these four IDs distinct and never recycle.
...
- **ERC-7484 (module registry attestations)** — `MODULE_TYPE_*` constants (`Typehashes.sol:45-55`) are `ModuleType` opaque IDs consumed by `IERC7484.check(address, ModuleType)`. Standard surface; Cork-private numeric IDs (`0xc0c0_0001..0xc0c0_0004`).

...
/// @title PreRolloverReferenceModule
/// @notice Reference pre-rollover hook that emits a balance snapshot for the calling rolloverContract.
///         Intended as a minimal, no-op example demonstrating the pre-hook interface.
...
contract PreRolloverReferenceModule is OnlyDelegatecall {
    /// @notice Emitted once per pre-rollover-hook invocation.
    /// @param rolloverContract Address that ran the hook (the rolloverContract via delegatecall).

...
/// @notice N-INV-FILLER-SETTLED-STICKY — continue-on-revert invariant suite: per-filler settled flag is monotone — never flips back to false.
/// @dev continue-on-revert mode (companion at test/invariant/failOnRevert/FillerSettledSticky.t.sol).
/// @custom:invariant N-INV-FILLER-SETTLED-STICKY
contract FillerSettledStickyContinueOnRevertTest is BaseTest {
    ...
    function invariant_settledLatchIsSticky() public view {
        assertFalse(
            ...
            "N-INV-FILLER-SETTLED-STICKY: fillerSettled regressed from true to false (loose)"
        );
    }
    ...
    function invariant_noDoublePayoutPerFiller() public view {
        assertFalse(
            ...
            "N-INV-FILLER-SETTLED-STICKY: same (orderId, filler) paid twice (loose)"
        );
    }
}

...
/// @notice N-INV-FACTORY-QUEUE-NONCE-SALT-UNIQUE — continue-on-revert suite:
///         successful trust-config queues use strictly increasing per-rolloverContract salts.
/// @dev Companion at test/invariant/failOnRevert/FactoryQueueNonceSalt.t.sol.
/// @custom:invariant N-INV-FACTORY-QUEUE-NONCE-SALT-UNIQUE
contract FactoryQueueNonceSaltContinueOnRevertTest is BaseTest {
    ...
    function invariant_factoryQueueNonceSaltMatchesExpected() public view {
        assertTrue(
            ...
            "N-INV-FACTORY-QUEUE-NONCE-SALT-UNIQUE: queued op id did not match nonce-derived salt (loose)"
        );
    }
    ...
    function invariant_factoryQueueSaltNeverReused() public view {
        assertTrue(
            ...
            "N-INV-FACTORY-QUEUE-NONCE-SALT-UNIQUE: queued op id was reused (loose)"
        );
    }
}

...
/// @notice INV-DEFAULT-ATTESTERS-FACTORY-SEEDED — continue-on-revert invariant suite: factory-seeded default attesters remain authoritative for new rolloverContracts.
/// @dev continue-on-revert mode (companion at test/invariant/failOnRevert/DefaultAttestersSeed.t.sol).
/// @custom:invariant INV-DEFAULT-ATTESTERS-FACTORY-SEEDED
contract DefaultAttestersSeedContinueOnRevertTest is BaseTest {
    /// @notice Seed handler.
    ...
    function invariant_preOverrideRolloverContractMirrorsFactorySeed() public view {
        assertFalse(
            ...
            "INV-DEFAULT-ATTESTERS-FACTORY-SEEDED: pre-override rolloverContract disagrees with factory seed (loose)"
        );
    }
}

...
/// @notice INV-FACTORY-DEFAULTS-MANAGED — continue-on-revert invariant suite:
///         factory-wide defaults rotate through DEFAULTS_MANAGER_ROLE and seed new rolloverContracts.
/// @dev continue-on-revert mode (companion at test/invariant/failOnRevert/FactoryDefaultsRotation.t.sol).
/// @custom:invariant INV-FACTORY-DEFAULTS-MANAGED
contract FactoryDefaultsRotationContinueOnRevertTest is FactoryDefaultsRotationInvariantBase {
    ...
    function invariant_factoryDefaults_liveGhostMatches() public view {
        assertFalse(
            ...
            "INV-FACTORY-DEFAULTS-MANAGED: live defaults diverged (loose)"
        );
    }
    ...
    function invariant_factoryDefaults_newRolloverContractsSeedLiveDefaults() public view {
        assertFalse(
            ...
            "INV-FACTORY-DEFAULTS-MANAGED: deployed rolloverContract seed mismatch (loose)"
        );
        assertFalse(
            defaultsHandler.freshDeployRejected(),
            "INV-FACTORY-DEFAULTS-MANAGED: fresh deploy rejected (loose)"
        );
    }
    ...
    function invariant_factoryDefaults_existingRolloverContractsUnaffected() public view {
        assertFalse(
            ...
            "INV-FACTORY-DEFAULTS-MANAGED: existing rolloverContract changed (loose)"
        );
    }
    ...
    function invariant_factoryDefaults_validSetsAccepted() public view {
        assertFalse(
            defaultsHandler.validSetRejected(),
            "INV-FACTORY-DEFAULTS-MANAGED: valid set rejected (loose)"
        );
    }
}

...
/// @notice N-INV-FILLER-DESTINATION-NONZERO — continue-on-revert invariant
///         suite: `fillerDestination[orderDigest][filler](,subFiller)` is
///         set exactly once on rollover record write, never re-written, and
///         never `address(0)` after the write completes.
/// @dev Companion at test/invariant/failOnRevert/FillerDestination.t.sol.
/// @custom:invariant N-INV-FILLER-DESTINATION-NONZERO
contract FillerDestinationContinueOnRevertTest is FillScaffold {
    ...
    ///         drops back to zero (loose mode).
    function invariant_fillerDestination_setOnceNeverZero() public view {
        assertFalse(
            destHandler.setOnceViolated(),
            "N-INV-FILLER-DESTINATION-NONZERO: fillerDestination was rewritten or zeroed after first non-zero observation (loose)"
        );
    }
    ...
    function invariant_fillerDestination_validFillsDoNotRevert() public view {
        assertFalse(
            ...
            "N-INV-FILLER-DESTINATION-NONZERO: valid exact/partial fill unexpectedly reverted (loose)"
        );
    }
    ...
    function invariant_fillerDestination_handlerNonVacuous() public view {
        assertGt(
            ...
            0,
            "N-INV-FILLER-DESTINATION-NONZERO: no successful destination-writing fill observed (loose)"
        );
    }
}

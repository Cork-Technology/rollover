...
/// @notice INV-CPT-CONTAINED — continue-on-revert invariant suite: rolloverContract holds zero srcCPT/dstCPT across a rollover bracket.
/// @dev continue-on-revert mode (companion at test/invariant/failOnRevert/RolloverContractCptContained.t.sol).
/// @custom:invariant INV-CPT-CONTAINED
contract RolloverContractCptContainedContinueOnRevertTest is BaseTest {
    /// @notice Cpt handler.
    ...
    function invariant_rolloverContractHoldsZeroCpt() public view {
        assertEq(
            IERC20(address(srcCpt)).balanceOf(rolloverContract), 0, "INV-CPT-CONTAINED: srcCPT leak"
        );
        assertEq(
            IERC20(address(dstCpt)).balanceOf(rolloverContract), 0, "INV-CPT-CONTAINED: dstCPT leak"
        );
    }
}

...
/// @notice N-INV-PARTIAL-RESIDUAL-RECONCILES-TO-TOTAL — fail-on-revert invariant suite: sum of partial fills and cPT-holder cancel equals the original orderSize.
/// @dev fail-on-revert mode (companion at test/invariant/continueOnRevert/PartialResidualReconciliation.t.sol).
/// @custom:invariant N-INV-PARTIAL-RESIDUAL-RECONCILES-TO-TOTAL
contract PartialResidualReconciliationFailOnRevertTest is BaseTest {
    ...
    function invariant_residualSumEqualsTotal() public view {
        assertFalse(
            ...
            "N-INV-PARTIAL-RESIDUAL-RECONCILES-TO-TOTAL: per-filler residual sum diverged from order escrow"
        );
    }
}

...
/// @notice F-PUSH — fail-on-revert invariant suite: Settler push-accounting ledger reconciles inflows against payout legs.
/// @dev fail-on-revert mode (companion at test/invariant/continueOnRevert/PushAccounting.t.sol).
/// @custom:invariant F-PUSH
contract PushAccountingFailOnRevertTest is BaseTest {
    /// @notice Push handler.
    ...
    /// @notice invariant: settler src cst reconciles.
    function invariant_settlerSrcCstReconciles() public view {
        ...
        assertGe(inAmt, outAmt, "F-PUSH: ghost-out exceeds ghost-in");
        assertEq(live, inAmt - outAmt, "F-PUSH: settler srcCST reconciliation drift");
    }
}

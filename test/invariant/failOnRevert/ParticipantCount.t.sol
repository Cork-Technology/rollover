...
/// @notice INV-PARTICIPANT-COUNT-MONOTONIC — fail-on-revert invariant suite: Settler participantCount per order is monotone non-decreasing.
/// @dev fail-on-revert mode (companion at test/invariant/continueOnRevert/ParticipantCount.t.sol).
/// @custom:invariant INV-PARTICIPANT-COUNT-MONOTONIC
contract ParticipantCountFailOnRevertTest is BaseTest {
    /// @notice Pc handler.
    ...
    function invariant_participantCountNonDecreasing() public view {
        ...
        for (uint256 i = 0; i < n; ++i) {
            ...
            assertGe(uint256(live), uint256(snap), "INV-PARTICIPANT-COUNT-MONOTONIC: regressed");
        }
    }
}
